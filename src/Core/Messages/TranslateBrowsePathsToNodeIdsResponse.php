...
/**
 * TranslateBrowsePathsToNodeIdsResponse - Response with resolved NodeIds
 */
final readonly class TranslateBrowsePathsToNodeIdsResponse implements IEncodeable, ServiceResponse
{
    private const int TYPE_ID = 556;

    /**
     * @param ResponseHeader $responseHeader Response header
     * @param BrowsePathResult[] $results Results for each path
    ...
    public function __construct(
        ...
        foreach ($results as $result) {
            if (!$result instanceof BrowsePathResult) {
                throw new InvalidArgumentException('Results must be BrowsePathResult instances');
            }
        }
    }
    ...
    public static function decode(BinaryDecoder $decoder): static
    {
        ...
        for ($i = 0; $i < $resultCount; $i++) {
            $results[] = BrowsePathResult::decode($decoder);
        }
